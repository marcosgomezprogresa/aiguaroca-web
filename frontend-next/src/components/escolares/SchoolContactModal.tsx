...
const SchoolContactModal: React.FC<SchoolContactModalProps> = ({ closeModal }) => {
  ...
  const activityHandler = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!activities) return toast('Por favor proporcione el numero de actividades');
    if (!kids) return toast('Por favor proporcione el numero de noches');
  ...
  return (
    <div className="w-full h-screen overflow-y-auto bg-black/30 fixed top-0 left-0 z-50 md:py-20">
      <div className="w-full bg-white custom-container max-w-6xl space-y-16 mx-auto border-2 border-neutral-100 shadow-lg md:rounded-3xl p-5 md:p-10 relative">
        ...
        <p className="text-center leading-tight text-5xl font-medium max-w-lg mx-auto">
        ...
        <form className="space-y-10" onSubmit={activityHandler}>
          ...
          <div className="flex flex-col md:flex-row gap-5 md:gap-10">
            <InputContainer
              ...
              onchange={(e) => {
                setActivities(normalizeNumericInput(e.target.value));
              }}
              value={activities}
              readOnly={loading}
            />
            <InputContainer
              ...
              onchange={(e) => {
                setKids(normalizeNumericInput(e.target.value));
              }}
              value={kids}
              readOnly={loading}
            />
          ...
          <div className="space-y-2 w-full text-lg">
            <TextAreaContainer
              icon={<PiBuildingOfficeBold className="mt-1" />}
